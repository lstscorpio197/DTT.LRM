...
$(document).ready(function () {
    $('#myTable').DataTable({
        ...
        ajax: {
            url: abp.toAbsAppPath('Liquidations/GetDataTable'),
            type: 'post',
            data: function (data) {
        ...
        columns: [
            ...
            {
                "data": "creator", className: "", render: function (text, display, data) {
                    return `<span>${text}</span>`;
                }
            },
            {
                "data": "createDateText", className: "", render: function (text, display, data) {
                    return `<span>${text}</span>`;
                }
            },
            {
    ...
    $(document).on('click', '#myTable tbody tr td:not(.btn-checkbox)', function () {
        ...
        window.location = abp.toAbsAppPath('Liquidations/CreateOrUpdate?liquidationId=' + id);
    })
});