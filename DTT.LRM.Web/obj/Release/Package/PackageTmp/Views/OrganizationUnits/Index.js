...
$(document).ready(function () {
    $('#myTable').DataTable({
        ...
        ajax: {
            url: abp.toAbsAppPath('OrganizationUnits/GetDataTable'),
            type: 'post',
            data: function (data) {
    ...
    $(document).on('click', '#myTable tbody tr td:not(.btn-checkbox)', function () {
        ...
        window.location = abp.toAbsAppPath('OrganizationUnits/CreateOrUpdate?organizationUnitId=' + id);
    })
});

$('.btn-trash').on('click', function () {
    abp.message.confirm(
        "Bạn có chắc muốn xóa các phòng ban đã chọn?", "Xóa?",
        function (isConfirmed) {
            if (isConfirmed) {
                ...
                $.each($('#myTable tbody tr input[type=checkbox]:checked'), function (i) {
                    let id = $(this).closest('tr').data('id');
                    _organizationUnitService.deleteById(id).done(function () {
                        if (count == i + 1) {
